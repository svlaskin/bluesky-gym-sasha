...
"""
using this one to train geo merge agent
"""
...
for episode in range(num_episodes):
    ...
    total_intrusions = np.append(total_intrusions, infos['DR001']['total_intrusions']) # append intrusions
    total_drift = np.append(total_drift, infos['DR001']['average_drift'])
    # print(infos)
    if episode % 10 == 0:
        print(f'episode: {episode}, avg rew: {total_rew[-100:].mean()}')
        # infodict = {'average_reward': total_rew[-100:].mean(),"average_intrusions":total_intrusions[-100:].mean()}
        log_episode(episode, total_rew[-100:].mean(), total_intrusions[-100:].mean(), total_drift[-100:].mean())
        save_models(model)
