...
class CentralisedMergeEnv(gym.Env):
    ...
    def zoom_and_pan(*, state, events, screen):
        """zoom and pan function."""
        for event in events:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_EQUALS:  # '+' key
                    state["zoom"] += state["zoom_step"]
                elif event.key == pygame.K_MINUS:  # '-' key
                    state["zoom"] = max(state["zoom_step"], state["zoom"] - state["zoom_step"])
                elif event.key == pygame.K_UP:
                    state["offset"][1] += state["pan_step"]
                elif event.key == pygame.K_DOWN:
                    state["offset"][1] -= state["pan_step"]
                elif event.key == pygame.K_LEFT:
                    state["offset"][0] += state["pan_step"]
                elif event.key == pygame.K_RIGHT:
                    state["offset"][0] -= state["pan_step"]

        # Render zoomed and panned surface
        zoomed_surface = pygame.transform.smoothscale(
            state["surface"],
            (
                int(state["surface"].get_width() * state["zoom"]),
                int(state["surface"].get_height() * state["zoom"]),
            ),
        )
        screen.blit(zoomed_surface, tuple(state["offset"]))
    ...
    # Checking all paits of AC for intrusions. If 
    ...
    def _render_frame(self):
        ...
        # zoom and pan
        print("AAAAAA",self.window)
        zoom_and_pan(state=state, events=self.events, screen=self.window)
