...
need_render = False
env_coop = sector_cr_v0.SectorCR_ATT_sas(render_mode='human') if need_render else sector_cr_v0.SectorCR_ATT_sas(render_mode=None)
env_uncoop = sector_cr_v0.SectorCR_ATT_sas_uncoop(render_mode='human') if need_render else sector_cr_v0.SectorCR_ATT_sas_uncoop(render_mode=None)
agents = env_coop.possible_agents
obs_dim = env_coop.observation_space(agents[0]).shape[0]
action_dim = env_coop.action_space(agents[0]).shape[0]
# n_agents = env.num_ac
n_agents = 25 # (total, of which 5 are made completely uncooperative)
