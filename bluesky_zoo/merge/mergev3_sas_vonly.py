...
"""
Drone edition with smaller TMA and different perf model.
V only action space.
"""
...
# DISTANCE_MARGIN = 20 # km, default val
DISTANCE_MARGIN = 0.1 # km
REACH_REWARD = 0#1
...
INTRUSION_DISTANCE =  2*0.15 # NM, for the extra PZ run 

SPAWN_DISTANCE_MIN = 3
SPAWN_DISTANCE_MAX = 12
...
# ALTITUDE = 350 # In FL
ALTITUDE = 3.28 # in FL, for 100m 

# Aircraft parameters
AC_SPD = 10 # base 150 for aircraft
AC_TYPE = "M600"
...
class MergeEnv_sas_vonly(ParallelEnv):
    metadata = {
        "name": "merge_v0_sas",
        "render_modes": ["rgb_array","human"],
         "render_fps": 120}

    def __init__(self, render_mode=None, n_agents=30, time_limit=150):
    ...
    def _gen_aircraft(self):
        ...
        for agent, idx in zip(self.agents,np.arange(self.num_ac)):
            ...
            bs.traf.cre(agent,actype="M600",acspd=AC_SPD,aclat=lat_ac,aclon=lon_ac,achdg=bearing_to_pos-180,acalt=ALTITUDE)
    ...
    def _get_observation(self):
        ...
        for agent in self.agents:
            ...
            observation = {
                ...
                "x_r": np.array(x_r[:NUM_AC_STATE]/1000),
                "y_r": np.array(y_r[:NUM_AC_STATE]/1000),
                "vx_r": np.array(vx_r[:NUM_AC_STATE]/15),
                "vy_r": np.array(vy_r[:NUM_AC_STATE]/15),
                "cos(track)": np.array(cos_track[:NUM_AC_STATE]),
                "sin(track)": np.array(sin_track[:NUM_AC_STATE]),
                "distances": np.array(distances[:NUM_AC_STATE])/25.
            }
    ...
    def _render_frame(self):
        ...
        max_distance = 15 # 30 decent # width of screen in km (300 default)
