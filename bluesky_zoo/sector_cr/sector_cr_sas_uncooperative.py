...
class SectorCR_sas_uncoop(ParallelEnv):
    ...
    def __init__(self, render_mode=None, n_agents=20, n_uncoop=10):
        ...
        self.agents_uncoop = self.agents[-n_uncoop:]
    ...
    def _do_action(self, actions):
        for agent in self.agents:
            ...
            if agent not in self.agents_uncoop:
                agent_uncoop = False
            else:
                agent_uncoop = True
            ...
            # print(speed_new)
            if not mvp_test and not agent_uncoop:
...
class SectorCR_ATT_sas_uncoop(SectorCR_sas_uncoop):
