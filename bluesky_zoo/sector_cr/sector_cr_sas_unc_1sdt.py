...
POLY_AREA_RANGE = (0.1, 0.3) # NM^2, factor of 4 roughly for km conversion -> 30m rpz
...
class SectorCR_sas_unc_quick(ParallelEnv):
    
    metadata = {
        "name": "sector_cr_v0_sas",
    ...
    def reset(self, seed=None, options=None):
        # print("so here is out culprit - is it bs.traf.reset?")
        bs.traf.reset()
        # print("no")
        # print("is it the areafilter?")
    ...
    def _kwikqdrdist_matrix_noisy(lata, lona, latb, lonb, noise):
    ...
    def _get_observation(self):
        ...
        for agent in self.agents:
            ...
            observation = {
                ...
                "airspeed": np.array([(airspeed)/25]),
                "x_r": x_r[:NUM_AC_STATE]/1000,
                "y_r": y_r[:NUM_AC_STATE]/1000,
                "vx_r": vx_r[:NUM_AC_STATE]/15,
                "vy_r": vy_r[:NUM_AC_STATE]/15,
                "cos(track)": cos_track[:NUM_AC_STATE],
                "sin(track)": sin_track[:NUM_AC_STATE],
                "distances": (distances[:NUM_AC_STATE])/25.
            }
    ...
    def _get_agents(self, n_agents):
        return [f'DR00{i+1}'.upper() for i in range(n_agents)]
    
    def _generate_polygon(self):
        # print("in poly right now")
...
class SectorCR_ATT_sas_unc_quick(SectorCR_sas_unc_quick):
    ...
    def _get_observation_noisy(self):
        ...
        for agent in self.agents:
            ...
            # self.noise_level = 10 # std of noise
            ...
            # vx = np.cos(np.deg2rad(ac_hdg)) * bs.traf.gs[ac_idx] + np.random.normal(0, self.noise_level, 1)[0]
            # vy = np.sin(np.deg2rad(ac_hdg)) * bs.traf.gs[ac_idx] + np.random.normal(0, self.noise_level, 1)[0]
            vx = np.cos(np.deg2rad(ac_hdg)) * bs.traf.gs[ac_idx]
            vy = np.sin(np.deg2rad(ac_hdg)) * bs.traf.gs[ac_idx]
            # print(vx, vx+np.random.normal(0, self.noise_level, 1)[0])

            ac_loc = fn.latlong_to_nm(CENTER, np.array([bs.traf.lat[ac_idx], bs.traf.lon[ac_idx]])) * NM2KM * 1000 # Two-step conversion lat/long -> NM -> m
            x = ac_loc[0] + np.random.normal(0, self.noise_level, 1)[0]
            y = ac_loc[1] + np.random.normal(0, self.noise_level, 1)[0]
